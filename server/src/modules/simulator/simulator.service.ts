...
export class SimulatorService {
  evaluateScript(input: EvaluateSimulationDto): SimulationResultPayload {
    ...
    if (
      opener.startsWith('today') ||
      opener.startsWith('in this video') ||
      opener.startsWith('hey guys')
    ) {
      hookStrength -= 2.0;
    }
    ...
    const audienceResonance = parseFloat(
      Math.min(9.9, Math.max(4.0, hookStrength * 0.95 + 0.4)).toFixed(1),
    );
    ...
    if (sentences.length > 1) {
      ...
      if (genericSentence) {
        hazards.push({
          ...
          whyReason:
            'Low information density; viewer already knows the topic and expects immediate tension.',
      ...
      if (longSentence) {
        hazards.push({
          ...
          fixSuggestion:
            'Split into 2 punchy 6-word sentences and insert a visual proof cut.',
        });
      }
    }
