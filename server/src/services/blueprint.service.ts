...
import { MinedChannelData } from './youtube.service';
...
export class BlueprintService {
  /**
   * Category & Theme Intelligence: Constructs creator-specific context and prompts Gemini for structured JSON
   */
  async generateDailyBriefing(channel: MinedChannelData): Promise<StructuredBlueprint[]> {
    ...
    try {
      ...
      console.warn('[Blueprint Service Warning] Gemini generation failed, using dynamic algorithm:', e);
      return this.generateAlgorithmicBlueprints(channel);
    }
  }
...
Median views: ${channel.medianViews.toLocaleString()}
...
`;
  ...
  private generateAlgorithmicBlueprints(channel: MinedChannelData): StructuredBlueprint[] {
    const medianV = channel.medianViews;
    ...
    return [
      ...
      },
      {
        ...
        dataProofReason: `Follow-up to your outlier upload with ${channel.outlierMultiplier}× view velocity.`,
